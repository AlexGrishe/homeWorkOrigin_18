document.write("Js working...");
document.write(" Gulp working...");
...
function test1() {
  ...
  while (str.indexOf('p') !== -1) str = str.replace('p', '');
  while (str.indexOf('s') !== -1) str = str.replace('s', '');
  while (str.indexOf('u') !== -1) str = str.replace('u', '');
    while (str.indexOf('m') !== -1) str = str.replace('m', '');
...
// данная функция необходима для корректного тестирования кода
function test2() {
  const reg = new RegExp('[ipsum,]', 'gui');

    return myLongStr.replace(reg, '');
